...
public abstract class BaseEnemyTile : Tile
{
    ...
    public virtual void TakeDamage(int damageAmount)
    {
        ...
        Debug.Log($"{this.gameObject.name} at {gridPosition} took {damageAmount} damage. Health: {currentHealth}/{maxHealth}");
    ...
    protected virtual IEnumerator FlashColorFeedback(Color flashColor, float duration)
    {
        ...
        yield return new WaitForSeconds(duration / 2);
        if (!isDefeated) sr.color = actualOriginalColor; else sr.color = Color.grey; // Ensure final color is correct
    }
    ...
    public virtual void DefeatEnemy()
    {
        if (isDefeated) return;
        isDefeated = true;
        Debug.Log($"{this.gameObject.name} at {gridPosition} is defeated.");
    ...
    public override void OnPlayerEnter(Player player)
    {
        ...
        Debug.Log($"{this.GetType().Name} at {gridPosition} was swapped WITH player. Enemy strikes for {baseAttackDamage} damage.");
        player.TakeDamage(baseAttackDamage);
    }

    // --- Abstract or Virtual methods for derived classes to implement AI ---
    public abstract void PerformAction(Player player, Vector2Int playerActualGridPos, Tile[,] gameGrid);
    public abstract List<Vector2Int> GetRelativeAttackPattern(); // Defines shape of attack
    ...
    public List<Vector2Int> GetCurrentAttackPatternWorldPositions()
    {
        ...
        {
            worldPositions.Add(this.gridPosition + relativePos);
        }
    ...
    {
        int distanceToPlayer = Mathf.Abs(playerPos.x - this.gridPosition.x) +
                               Mathf.Abs(playerPos.y - this.gridPosition.y);
        ...
        if (distanceToPlayer <= detectionRange)
        {
            ...
            Vector2Int directionToPlayer = playerPos - this.gridPosition;
    ...
    {
        if (GameManager.Instance == null || GameManager.Instance.IsAnimating()) return;
        ...
        {
            Vector2Int nextPos = this.gridPosition + dir;
            if (GameManager.Instance.InBounds(nextPos))
            {
                Tile tileAtNextPos = gameGrid[nextPos.x, nextPos.y];
                if (tileAtNextPos.type == TileType.Empty || tileAtNextPos is EnvironmentTile)
                {
        ...
        if (potentialMoveDirs.Count > 0)
        {
            ...
            GameManager.Instance.AttemptEnemyTileSwap(this, this.gameObject, bestMoveDir);
        }
    }
}